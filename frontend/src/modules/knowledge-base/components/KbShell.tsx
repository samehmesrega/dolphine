...
export default function KbShell({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen bg-slate-50" dir="rtl">
      ...
      <button onClick={() => setMobileOpen(true)} className="md:hidden fixed top-3 right-3 z-30 bg-slate-800 text-white p-2.5 rounded-xl shadow-lg">
        <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M4 6h16M4 12h16M4 18h16" /></svg>
      </button>

      <aside className={`fixed inset-y-0 right-0 z-50 md:static md:z-auto transition-transform duration-200 md:translate-x-0 ${mobileOpen ? 'translate-x-0' : 'translate-x-full'} ${collapsed ? 'w-16' : 'w-64'} bg-slate-800 text-white flex flex-col`}>
