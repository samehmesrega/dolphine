...
export default function TaskRulesPage() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      setAction(''); setStatusSlug(''); setAfterHours(12);
      invalidate();
    },
  });
  ...
  const handleCreate = (e: React.FormEvent) => {
    e.preventDefault();
    if (!action.trim() || !statusSlug || afterHours < 1) return;
    createMutation.mutate({ action: action.trim(), statusSlug, afterHours });
  };
  ...
  const handleUpdate = (e: React.FormEvent) => {
    ...
    updateMutation.mutate({
      id: editId,
      body: { action: editAction.trim(), statusSlug: editSlug, afterHours: editHours },
    });
  };
  ...
  return (
    <div>
      ...
      <p className="text-slate-500 text-sm mb-6">
        أنشئ قاعدة لإنشاء مهام تلقائية. عندما يكون الليد في حالة معينة ويمر عليه وقت محدد بالساعات،
        يتم إنشاء مهمة بالفعل المحدد تلقائياً للموظف المسؤول.
      </p>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-6 mb-6">
        ...
        <form onSubmit={handleCreate}>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
            <div>
              <label className="block text-xs text-slate-500 mb-1">افعل (عنوان المهمة)</label>
              <input
                type="text"
                value={action}
                onChange={(e) => setAction(e.target.value)}
                className={inputClass}
                placeholder="مثال: تواصل مع العميل"
                required
              />
            </div>
            <div>
              <label className="block text-xs text-slate-500 mb-1">عندما حالة الليد</label>
              <select
                value={statusSlug}
            ...
            <div>
              <label className="block text-xs text-slate-500 mb-1">بعد (ساعات)</label>
              <input
                type="number"
                min={1}
                value={afterHours}
                  ...
                  <div className="flex items-center justify-between gap-4">
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-slate-500">
                        ...
                        {' · '}بعد{' '}
                        <span className="font-medium text-slate-700">{rule.afterHours}</span>{' '}ساعة
                      </p>
                    </div>
