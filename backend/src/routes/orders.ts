...
router.post('/', uploadSingle, async (req: Request, res: Response) => {
  ...
  // إذا الليد ما له عميل مربوط، نربطه تلقائياً حسب رقم الفون (إنشاء/البحث عن عميل)
  if (!lead.customerId) {
    const phoneNorm = lead.phoneNormalized || normalizePhone(lead.phone);
    if (!phoneNorm) {
