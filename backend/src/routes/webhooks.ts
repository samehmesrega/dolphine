...
function pickPhone(obj: Payload): string | null {
  ...
  for (const k of Object.keys(obj)) {
    ...
    if ((key.includes('phone') || key.includes('tel') || key.includes('mobile') || key.includes('رقم') || key.startsWith('tel'))) {
      const val = obj[k];
...
router.post('/leads/:token', async (req: Request, res: Response) => {
  ...
  if (!connection) {
    console.log('[webhook] رابط غير صالح أو منتهي، token:', token?.slice(0, 8) + '...');
  ...
  const customer = await prisma.customer.upsert({
    ...
    update: { whatsapp: raw.whatsapp as string | undefined, email, address },
    create: {
